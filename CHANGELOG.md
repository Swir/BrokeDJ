...
- The research path is still not connected to `Engine::process()`. Algorithm-latency metadata is exposed for the next scheduling step, but full deck latency compensation, live rate/pitch automation, reviewed listening and hardware deadline/underrun qualification remain open.
...
- Device-rate bridge PR #16 is under exact-final-head qualification and must not merge until Linux sanitizer plus Windows x64 build/full CTest/audio-diagnostics/native GUI smoke/staging/artifact checks are green on the same final head.
