...
- Streaming-hardening checkpoint `1d17dbb230fd3740e712ed330475e20cf24f78f7` passed Linux sanitizer/progress/CTest and Windows configure/build, but Windows CTest run `35499161872` correctly exposed one new test-design regression: the compressed Reverse/Slip stress rendered long enough for the hidden Slip timeline to reach EOF, leaving no active playback that could close the asserted starvation episode. Existing tests stayed green. The follow-up head shortens that burst so EOF cannot invalidate the refill assertion and waits for the exact interpolation neighbourhood before checking recovery; it requires a fresh exact-head gate.
...
- The corrected final head of PR #44 requires exact-head Linux ASan/UBSan + generated-progress + full CTest and Windows x64 configure/build/full CTest/audio diagnostics/native GUI smoke/silent device probe/staging before merge.
