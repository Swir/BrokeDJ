...
- `main` now includes squash-merged PR #105 at `8e5ee002bf9a1c847e76d1fdc15bb6cb01af2bae` (`Make the first-Beta workstation panel usable at every supported size`). The exact code head qualified before merge was `b62b4935e488b6816423e21ce2ff33e8307d7749`.
...
- The user-facing workstation now keeps the real four-channel central mixer and channel faders visible throughout the supported workstation size range, adds independent per-deck MIX/GRID views, larger performance waveforms, sampled pre-fader peak strips, one-click Library plus separate Session controls, and explicit non-owning UI handles instead of child-order/caption discovery.
...
- **M2:** reviewed-grid performance controls, continuous Sync, variable-tempo grids, Hot Cues, Beat Jump, REV/SLIP, bounded JOG/SCRATCH and the opt-in Signalsmith-backed key-lock research lifecycle are implemented. Native `MainComponent`/DeckPanel integration covers PLAY/rate/LOOP/CUE 0, live seek and fail-closed clip replacement. The merged Beta workstation keeps this performance surface accessible across supported sizes. Representative real-music BPM/key review, key-lock listening/latency qualification and physical controller workflows remain open.
...
Exercise the smoke-qualified Windows x64 Beta Preview as the user-facing workstation candidate and use the integrated guided runner against that exact executable to complete the genuine M1–M4 manual Windows 11/hardware/listening/library evidence. Fix any real panel, import, playback, device, recording or session regression before widening M5+ scope. Once those first-Beta gates pass, publish the appropriately labelled Beta with source/notices/checksum and known-issues review rather than treating automated CI alone as release qualification.
