...
- Push-main Build and test #506 and Native library scale smoke #176 started for `a366d73…`; they were still queued when the next finish-first M4 package was opened. Do not reinterpret the earlier exact-head PR gate as a post-merge hardware/listening qualification.
- Active package is draft PR #101, branch `fix/m4-exact-app-witness-binding`. Code/docs head immediately before this checkpoint is `2668c2a4f8a31a22ce9700360b9bf13a0d2ab86b`; this checkpoint commit advances the branch once more, so the PR's newest exact-head workflows are the merge authority.
...
- The disposable fixture self-test now also covers exact-path identity semantics in both supported PowerShell-host workflow runs; existing unique-WAV, privacy, closed-schema and two-pass stability contracts remain intact.
...
- **PR #101:** every workflow must pass on the newest exact branch head. Any PowerShell 5.1/7 parser/self-test, evidence-contract, build, package, native-scale, UI or existing regression failure must be repaired before merge.
...
- **M4:** perform and review the user-controlled Windows 11 library/session witness against an exact staged executable. Exact-process ownership reduces executable ambiguity but does not fabricate launch/resize/import/search/tag/playlist/history/duplicate/missing/relocate/backup/restore/session evidence.
- **Release:** no Alpha/Beta/Stable Release until the documented gate for that scope is satisfied.

## Next largest step

Let PR #101's newest exact-head workflows exercise the exact-process witness on both PowerShell hosts and the normal build/package matrix. Repair every regression before integration. Keep the Beta scope frozen: once the witness tooling is green, the largest remaining gate is the real Windows 11 M4 connected library/session run itself, followed by physical M1 device/four-output checks and the remaining M2–M3 listening/hardware evidence. If those manual gates cannot be executed in the current environment, preserve the verified software baseline instead of inventing substitute evidence.
