...
- Active package is branch `fix/m4-fixture-stability-verification`. This checkpoint strengthens the connected M4 verifier from one filesystem/database observation to two bounded passes around two privacy-safe aggregate snapshots. A pull request and new exact-head run are required before integration.
