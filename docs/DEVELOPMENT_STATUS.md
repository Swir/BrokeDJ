...
- Latest code checkpoint before this documentation update: `55e91a12f35be768837831e8fe1cbdec4e927458`; exact-head PR run `35483733571` was started for that code head. This documentation update creates a newer PR head and therefore also requires its own exact-head checks before any merge.
- Active package: Reverse/Slip message-thread ownership rules plus direction-aware bounded read-ahead for streamed reverse playback.
...
- `PerformanceDeckOwnerTests` covers Reverse/Slip activation, beat-loop rejection, whole-track compatibility, Hot Cue / Beat Jump / Sync exits, invalid-deck rejection and clip-reset cleanup.
...
- Deeper prefetch now follows the observed direction of changed requested chunks. Ordinary forward playback keeps forward deep read-ahead; a sustained reverse/slip cursor can build cache behind the playhead instead of repeatedly biasing work ahead of it.
...
- `DecoderTests` now establishes a distant forward request, verifies forward deep prefetch, then moves the request backward and requires a deeper preceding chunk to become resident. The previous forward-only worker would not satisfy that regression fixture.
