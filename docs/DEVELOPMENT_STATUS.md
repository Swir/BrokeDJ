...
- Active development: draft PR #68, branch `fix/m4-safe-library-restore`. Latest functional head before this checkpoint documentation commit: `768332d3a455a7ee97053e75c89eefade336b843`.
- The package closes a data-safety hole in library restore: a SQLite-valid file could previously claim the current `user_version` while missing BrokeDJ tables, pass `quick_check`, and replace the live snapshot before the later code discovered the mismatch.
- Library integrity validation now combines SQLite `quick_check`, exact current schema version, required BrokeDJ table/column probes, and `foreign_key_check`.
- Library backup now validates the live database before copying and validates the produced backup snapshot before reporting success.
- Library restore now copies the selected source into a private temporary database, migrates and validates that staged snapshot before touching the live database, and captures a validated rollback snapshot before installation. A failed installation attempts to restore and revalidate the previous live snapshot.
- Deterministic library tests add a `user_version=2` malformed-schema fixture that is SQLite-valid but lacks BrokeDJ tables, require rejection without live-data loss, verify generated backups can be reopened with full integrity, and verify v1 backups are staged/migrated before installation.
- A focused local C++20/system-SQLite safety smoke passed for fresh open, verified backup, malformed-current-schema restore rejection with live-data preservation, and v1 staged migration. This is not a Windows/audio-device or listening test.
- This checkpoint documentation commit changes the PR head. Exact-final-head repository CI must pass again before the PR can be considered for integration.
...
- Backup success now means both the source and produced SQLite snapshot passed BrokeDJ integrity validation; it does not imply external storage durability or disaster-proof persistence.
...
- PR #68 remains development-only until required exact-final-head GitHub Actions checks are green for the same final head. The earlier local focused smoke is supporting evidence only.
...
Require green exact-final-head PR #68 build/test and focused native-library checks; fix any regression before integration. Keep the Windows 11 manual M4 library/recovery witness explicit, then finish remaining internally closable M4 acceptance work before widening optional DSP scope.
