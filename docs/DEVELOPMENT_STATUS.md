...
- Active branch: `feat/package-beta-witness-runner`; draft PR **#110** targets `main`. Product/package head immediately before this status-only checkpoint is `b46847e9094100d3d36f268614cfd0bab8d7825e`; this checkpoint advances the branch, so only exact final-head CI may authorize merge.
...
- PR #110 stages `BETA-WITNESS-RUNNER.ps1`, its guide and `START-BETA-QUALIFICATION.cmd` beside the exact packaged `BrokeDJ.exe`. The launcher binds to the adjacent executable/runner names, keeps accepted evidence in `%USERPROFILE%\Documents\BrokeDJ-Beta-Evidence`, and invokes the expected system Windows PowerShell executable by its `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe` path instead of relying on `PATH`/current-directory command resolution.
- The launcher does not elevate, change PowerShell execution policy, auto-start ordinary playback, enable microphone input or begin recording. Missing app/runner/system PowerShell paths fail closed before the guided session begins.
- Package-smoke now requires the downloaded staged tree to contain the runner/guide/launcher. After extraction of the **real portable ZIP** into the existing relocated Unicode/spaces path, it runs the runner's prompt-free tool-resolution fixture self-test and verifies launcher binding before the existing no-audio GUI/device-probe smoke and post-smoke byte verification continue.
- Exact product-head `b46847e9094100d3d36f268614cfd0bab8d7825e` already passed M1 witness-tool **#83**, M2 witness-tool **#78**, M3 witness-tool **#73** and Beta witness runner **#35**. At this checkpoint Beta qualification **#51** was still running, Build and test **#559** was queued and Native library scale **#229** was pending. These product-head runs become predecessor evidence after this status-only commit; exact final-head reruns remain the merge authority.
