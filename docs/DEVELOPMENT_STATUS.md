...
- Default branch baseline: `main` at `3e3ee9b96e18ccc62de43b20c4976bf13aa4116c`. PR #64 (`M4: wire playback history and bounded legacy hash backfill`) is integrated after exact-head workflow `35568715694` completed successfully; post-merge main workflow `35569629015` also completed successfully.
- Active development: PR #65, branch `feat/m4-library-scale-hardening`. Functional package head before this checkpoint documentation commit: `fefe8e4ffa44220d36422e821380c8e100f94556`.
- First exact-head package run `35569946131` exposed one Windows-only regression in missing-file classification: 33/34 Windows CTests passed and `library_runtime_store` failed because `std::filesystem::is_regular_file(..., error_code)` can report a normal nonexistent path with `no_such_file_or_directory` on Windows.
- The current functional head fixes that regression by treating only no-error/ENOENT/ENOTDIR non-regular results as confirmed missing while permission/I/O errors remain retryable failures; deterministic classification coverage was added. Exact-head workflow `35570855795` was queued for that functional head when this checkpoint was written. This documentation commit changes the PR head, so a fresh exact-head run is required before merge.
...
- Legacy hash maintenance distinguishes confirmed missing/non-regular sources from inspection failures. Windows/Linux differences for ENOENT/ENOTDIR are normalized, while permission/I/O inspection errors increment the failure diagnostic and do not mark the track missing.
