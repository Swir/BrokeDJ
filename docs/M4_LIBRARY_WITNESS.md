...
5. **Duplicate and missing review** — import the byte-identical generated duplicate and exercise `is:duplicate`. Then move the primary fixture into the printed relocation directory, choose **Library → Refresh file status**, wait for the bounded background refresh summary, and exercise `is:missing`. Confirm the moved primary is discovered without first trying to load it, the duplicate/missing workflows remain review-only, and source media is not deleted or overwritten. If the summary reports unresolved filesystem probes, treat that as a witness blocker until understood.
...
Automated CI covers SQLite migrations, bounded search, tags/playlists/history, content hashing, duplicate/missing directives, paged file-presence reconciliation, moved-file rebinding, waveform/analysis cache ownership, bounded session snapshots and native library backup/restore recovery against synthetic data. File-presence reconciliation snapshots ID/path/old-state in bounded pages, performs filesystem probes away from SQLite write transactions, and conditionally updates only the unchanged record identity; the GUI runs the pages on its maintenance worker, never on the audio callback. The preflight additionally proves that the exact selected executable passes its no-audio native GUI resize/geometry contract and that the same PowerShell host can prepare a valid disposable library fixture before the human workflow begins. Neither automated layer replaces the connected Windows 11 **user workflow**.
