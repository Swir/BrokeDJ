...
For M4, the accepted input is the current **schema-2** library/session witness. That validator requires `witness.interactionBinding = tracked-exact-app-process-v1` and `witness.fixtureStateVerification = two-pass-stable-aggregate-v1`, so a legacy schema-1 M4 file cannot be carried forward into a Beta summary merely because its eight booleans and executable hash look valid. The M4 witness must be rerun against the exact candidate executable.
...
- `BrokeDJ-M4-Library-Witness.json` generated by the current exact-process-bound schema-2 M4 witness;
...
Revalidation fails if the executable, source commit, device probe or any M1–M4 evidence file has changed, if any individual witness no longer validates, if the summary contains unexpected fields, or if its privacy contract is altered. In particular, replacing the M4 file with legacy schema-1 evidence fails at the M4 validator before the composed Beta summary can be accepted. Validation is intentionally permitted in CI because it does not generate new human evidence.
