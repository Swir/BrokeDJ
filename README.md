...
| Musical analysis | Bounded offline BPM, beat-grid anchor and major/minor key analysis after a successful load; local privacy-preserving analysis cache v2; source-identity-bound manual grid overrides; compact beat-zero/base-BPM correction controls; waveform beat-line overlay; JUCE-independent editable variable-tempo grid model | BPM/grid and key consume one background decode pass and can independently decline low-confidence material. The compact editor corrects beat zero and segment-0 BPM while preserving later tempo-change boundaries. Full tempo-segment editing, representative real-music validation and sync remain open. |
...
| Interface | Four deck panels, waveforms with validated beat-line overlays, per-deck BPM/grid/key status, compact manual grid correction, drag/drop, audio settings and clickable `by Swir` credit | Native JUCE source; Windows CI builds and headless GUI lifecycle smoke tests, while clean-machine/manual usability qualification remains separate. |
...
Runtime diagnostics use JUCE's application log directory under `BrokeDJ/BrokeDJ.log`. Logs can contain local filenames; review them before posting an issue. Analysis cache and manual-grid payloads themselves intentionally omit the raw source pathname.
...
| `src/app/` | Native interface, decoder/read-ahead workers, musical-analysis/cache/grid-override adapter and device integration |
