...
| Musical analysis | Bounded offline BPM, beat-grid anchor and major/minor key analysis after a successful load; local privacy-preserving analysis cache v2; source-identity-bound manual grid overrides; beat-zero/base-BPM controls plus a native add/move/replace/remove variable-tempo segment editor with bounded Undo/Redo; waveform beat-line overlay | BPM/grid and key consume one background decode pass and can independently decline low-confidence material. Reviewed grids drive beat loops, Hot Cue quantization, Beat Jump and bounded continuous Sync. Representative real-music validation remains open. |
...
| Interface | Four deck panels, waveforms with validated beat-line overlays, per-deck BPM/grid/key status, native reviewed tempo-map editing, reviewed-grid beat loops, eight persistent Hot Cue pads, Beat Jump ±1/2/4/8/16, REV/SLIP, bounded native JOG/SCRATCH, explicit MASTER plus continuous SYNC lock, drag/drop, audio settings and clickable `by Swir` credit | Native JUCE source; Windows CI builds and no-audio lifecycle/resize smoke tests, while clean-machine/manual usability and controller qualification remain separate. Continuous Sync is message-thread control maintenance with bounded phase correction, not an audio-callback beat scheduler. |
...
| Reviewed-grid performance controls | Native 1/2/4/8/16-beat loops, eight source-bound persistent Hot Cue pads, Beat Jump ±1/2/4/8/16, REV/SLIP, bounded JOG/SCRATCH and explicit MASTER/continuous SYNC lock; ordinary playback remains independent of analysis |
...
When a valid beat grid exists, **GRID ZERO** shifts the grid anchor and **GRID BPM** corrects its base tempo. A `GRID*` marker means a local manual override is active. Corrections are stored outside the audio callback for the exact current file identity; **Reset grid** removes the override and returns to the detector result. **TEMPO MAP** opens the reviewed variable-tempo editor, where later boundaries can be added at the playhead, moved, replaced or removed with bounded Undo/Redo. Persistence remains off the audio callback.

With a reviewed grid, **BEAT LOOP** can arm 1/2/4/8/16-beat musical loops from the current transport. `HC1`–`HC8` store and recall per-track Hot Cues; Shift+click clears a pad. New cues quantize to the nearest beat only when a reviewed grid is available. **JUMP - / JUMP +** move by the selected 1/2/4/8/16-beat distance while preserving fractional beat phase. **REV/SLIP** and the bounded **JOG/SCRATCH** strip use the production transport with fail-closed ownership rules. Select one reviewed-grid deck as **MASTER**, then toggle **SYNC** on a follower: BrokeDJ performs a bounded initial alignment and maintains tempo/phase from the message thread at a bounded cadence. Small phase error stays inside a deadband; Beat Loop or Reverse/Slip releases the follower lock, and an active Jog temporarily suspends maintenance instead of fighting the platter.
