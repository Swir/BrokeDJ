...
void run() {
    ...
    check(selector.prepare(sourceRate, deviceRate, 4096, 4.0),
          "selector prepares outside realtime window");

    double cursor = sourceRate * 2.0 + 0.5;
    constexpr Snapshot controls{1.25, -2.0f, true};
    check(selector.stage(clip, cursor, false, controls),
          "selector stages key-lock outside realtime window");
    ...
    for (int block = 0; block < 80; ++block) {
        ...
        check(selector.render(clip, cursor, false,
                              left.data(), right.data(), frames, next, audible),
              "selector warmup renders");
        check(selector.lastRenderPath() == Selector::RenderPath::stretch,
              "selector warmup remains on key-lock path");
        cursor = next;
    }
    ...
    for (int block = 0; block < measuredBlocks; ++block) {
        double next = cursor;
        double audible = cursor;
        if (!selector.render(clip, cursor, false,
                             left.data(), right.data(), frames, next, audible)
            || selector.lastRenderPath() != Selector::RenderPath::stretch
            || selector.lastFallbackReason() != Selector::FallbackReason::none
            || !(audible < next)) {
            trackHeap.store(false, std::memory_order_seq_cst);
            throw std::runtime_error("measured selector window keeps staged key-lock path");
        }
        cursor = next;
    }
    trackHeap.store(false, std::memory_order_seq_cst);
    ...
    check(observedAllocations == 0,
          "deck selector render performs no heap allocation after staging");
    check(observedDeallocations == 0,
          "deck selector render performs no heap deallocation after staging");
    check(std::all_of(left.begin(), left.end(), [](float value) { return std::isfinite(value); })
          ...
          "selector realtime output remains finite");
    ...
    check(elapsedNs > 0 && std::isfinite(nsPerFrame) && nsPerFrame > 0.0,
          "selector diagnostic callback cost is finite and positive");

    std::cout << "METRIC deck_selector_blocks=" << measuredBlocks
              << " source_rate=" << sourceRate
              << " device_rate=" << deviceRate
              << " playback_rate=" << controls.playbackRate
              << " pitch_semitones=" << controls.pitchSemitones
              << " latency_frames=" << selector.reportedDeviceOutputLatencyFrames()
...
int main() {
    try {
        run();
        std::cout << "PASS: " << checks << " deck selector realtime checks\n";
        return EXIT_SUCCESS;
