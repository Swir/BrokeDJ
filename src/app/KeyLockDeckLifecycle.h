...
// deterministically before play resumes. This intentionally favors reliable
// audio over seamless live key-lock automation until that harder handoff is
...
public:
    ...
    [[nodiscard]] ServiceStatus service(std::size_t deck, double positionSeconds,
        ...
        if (!state.clip || !state.clip->valid()) return ServiceStatus::noClip;
