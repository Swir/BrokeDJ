...
constexpr int overrideSchema = 1;
constexpr std::int64_t maxCacheBytes = 64 * 1024;
constexpr std::int64_t maxOverrideBytes = 64 * 1024;
...
TrackBeatGridOverrideStore::TrackBeatGridOverrideStore(juce::File rootIn)
    : root(std::move(rootIn)) {}

juce::File TrackBeatGridOverrideStore::defaultRoot() {
    return juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
        .getChildFile("BrokeDJ")
        .getChildFile("beat-grid-edits-v1");
}

juce::File TrackBeatGridOverrideStore::overrideFileFor(const juce::File& source) const {
    return root.getChildFile(hex64(fnv1a64(source.getFullPathName())) + ".grid");
}

bool TrackBeatGridOverrideStore::load(const juce::File& source, broke::BeatGrid& grid) const {
    grid = {};
    if (!source.existsAsFile()) return false;
    const auto stored = overrideFileFor(source);
    if (!stored.existsAsFile() || stored.getSize() <= 0 || stored.getSize() > maxOverrideBytes)
        return false;

    const auto fields = parseFields(stored.loadFileAsString());
    const juce::String empty;
    if (fields.getValue("schema", empty).getIntValue() != overrideSchema
        || fields.getValue("fileSize", empty).getLargeIntValue() != source.getSize()
        || fields.getValue("modifiedMs", empty).getLargeIntValue()
            != source.getLastModificationTime().toMilliseconds()) {
        return false;
...
bool TrackBeatGridOverrideStore::store(const juce::File& source,
                                       const broke::BeatGrid& grid) const {
    ...
    const auto target = overrideFileFor(source);
    juce::TemporaryFile temporary(target);
    if (!temporary.getFile().replaceWithText(payload, false, false, "\n")) return false;
    return temporary.overwriteTargetFileWithTemporary();
}

bool TrackBeatGridOverrideStore::erase(const juce::File& source) const {
