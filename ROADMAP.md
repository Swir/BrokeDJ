...
3. Wire the qualified deck-owned selector into the smallest opt-in production Engine/deck boundary: keep prepare/re-prime outside the callback, preserve per-deck EQ/FX/cue/meter semantics, apply reported algorithm latency to audible scheduling, retain immediate production fallback and qualify load/seek/loop/rate/pitch/fallback transitions before exposing any UI control.
...
The opt-in time-stretch research path remains outside ordinary playback. Its JUCE-independent layers cover processor configuration/latency metadata, bounded fractional deck clocking, exact bounded reads from real in-memory or streamed BrokeDJ sources, explicit conversion from stretched source-rate output to device-rate output through a bounded FIFO and prepared band-limited SRC, Engine-facing lifecycle staging and a deck-owned path selector. The bridges consume caller-provided production fallback samples, keep audible transport separate from prefetch, expose algorithm-latency metadata and fail back rather than emitting stale stretch/FIFO data after bypass/discontinuity/control changes. The deck selector owns off-callback stage/disarm intent and adds a short prepared de-click when the selected path changes, while changed controls, clip replacement, loop changes, cursor jumps and incompatible source rates still require explicit restaging. Deterministic tests cover independent time ratio/pitch, reset/seek history, fractional transport, loop/EOF behavior, cache starvation/refill, 44.1→48 and 96→48 conversion, fallback behavior, path switching and warmed zero-heap processing. Passing these tests establishes a qualified integration candidate; it does not establish production Engine playback, fully compensated deck latency, seamless perceptual switching, listening quality, supported hardware latency or release readiness.
