...
| M2 | Performance decks | Validated beat/tempo/key analysis, editable grids including tempo changes, key lock, hotcues, beat loops, slip/reverse/scratch and de-clicked transitions | De-clicking, smoothed rate changes, hybrid Catmull-Rom/band-limited windowed-sinc rate conversion, bounded long-track read-ahead, codec stress, starvation/refill telemetry and objective render/real-time contract tests are implemented; the opt-in time-stretch/key-lock research stack now includes the processor wrapper, bounded deck clock and a real Clip/StreamCache source bridge under PR #15 qualification, but it is not connected to production deck playback; milestone still open |
...
3. Qualify PR #15's real-source bridge on its exact final head, then design the opt-in Engine-facing output/FIFO integration: explicitly resolve source-rate versus device-output-rate conversion, compensate reported processing latency, de-click enable/bypass/seek/loop/load/starvation transitions and crossfade immediately back to the current production rate converter on processor/source failure. Keep it opt-in until render, realtime and reviewed Windows listening evidence support promotion.
...
The opt-in time-stretch research path remains outside ordinary playback. Its three JUCE-independent layers now cover processor configuration/latency metadata, bounded fractional deck clocking and exact bounded reads from real in-memory or streamed BrokeDJ sources. Deterministic tests cover independent time ratio and pitch, reset/seek history, fractional transport, loop/EOF behavior, cache starvation/refill and warmed zero-heap processing. Passing those tests establishes only a candidate integration path; it does not establish Engine playback, compensated latency, seamless fallback, listening quality, supported hardware latency or release readiness.
